...
class ReportMgr(ReportMgrBase):
    ...
    def _report_step(self, lr, step, train_stats=None, valid_stats=None):
        ...
        if train_stats is not None:
            self.log('Train xsent: %g' % train_stats.xent())
        ...
        if valid_stats is not None:
            self.log('Validation xsent: %g' % valid_stats.xent())
            self.log('Validation perplexity: %g' % valid_stats.ppl())
            self.log('Validation accuracy: %g' % valid_stats.accuracy())
            self.log('Validation ss accuracy: %g' % valid_stats.ss_accuracy())
...
class Statistics(object):
    ...
    * elapsed time
    ...
    def __init__(self, loss=0, n_words=0, n_correct=0, extra=None):
        self.loss = loss
    ...
    def update(self, stat, update_n_src_words=False):
        ...
        if stat.extra and isinstance(stat.extra, dict):
            if not self.extra:
                self.extra = {}
            for k,v in stat.extra.items():
                if isinstance(v, int) or isinstance(v, float):  # sum of loss
                    self.extra[k] = self.extra.get(k, 0) + v
                else:
                    if not isinstance(v, list):                 # list
                        v = v.tolist()                          # [n, seq_len]
                    if k not in self.extra.keys():
                        self.extra[k] = v
                    else:
                        self.extra[k].extend(v)
    ...
    def ss_accuracy(self):
        """ compute ss accuracy """
        if self.extra and 'ss_correct' in self.extra and 'ss_num' in self.extra:
            return 100 * (self.extra['ss_correct'] / self.extra['ss_num'])
        else:
            return 0
    ...
    def output(self, step, num_steps, learning_rate, start):
        ...
        if self.extra:
            extra_info = "Step {}/{}; ".format(step, num_steps)
            for k, v in self.extra.items():
                if k.endswith('loss'):
                    if isinstance(v, int) or isinstance(v, float):
                        extra_info += "{}: {:.3f}; ".format(k, v / self.n_words)
            extra_info += "{}: {:.3f}; ".format('ss_accu', self.ss_accuracy())
    ...
    def log_tensorboard(self, prefix, writer, learning_rate, step):
        ...
        writer.add_scalar(prefix + "/ss_accuracy", self.ss_accuracy(), step)
        ...
        if self.extra:
            for k, v in self.extra.items():
                if isinstance(v, int) or isinstance(v, float):
                    writer.add_scalar(prefix + "/{}".format(k), v / self.n_words, step)
